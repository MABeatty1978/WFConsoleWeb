...
from wfconsoleweb.config.models import (
    AdminUser,
    StationConfig,
    APIKey,
    DisplaySettings,
...
from wfconsoleweb.config.encryption import get_encryption_manager
...
class DisplaySettingsRequest(BaseModel):
    ...
    distance_unit: str = "km"  # or "mi"
    preferred_forecast_source: str = "tempest"  # "tempest", "sager", or "zambretti"
...
async def get_display_settings(
    ...
    return {
        ...
        "distance_unit": settings.distance_unit or "km",
...
async def update_display_settings(
    ...
    try:
        ...
        display_query = db.query(DisplaySettings)
        if current_user:
            display_query = display_query.filter(
                DisplaySettings.user_id == current_user.id,
                DisplaySettings.device_key == x_device_key,
            )
        else:
        ...
        display = display_query.first()
        if not display:
            display = DisplaySettings()
            db.add(display)

        display.user_id = current_user.id if current_user else None
        ...
        display.distance_unit = settings.distance_unit if settings.distance_unit in {"km", "mi"} else "km"
        display.preferred_forecast_source = (
            settings.preferred_forecast_source
            if settings.preferred_forecast_source in {"tempest", "sager", "zambretti"}
            else "tempest"
        )
        ...
        logger.info(f"Display settings updated by {actor}")
