...
export default function RainfallPanel({ wxSummary }: Props) {
  ...
  const rateDisplay =
    rateMMph !== null
      ...
      : "--";
  const todayVsYesterday = today - yesterday;
  const yearlyProgress = year > 0 ? (month / year) * 100 : null;
  const rainIntensity =
    rateMMph === null || rateMMph <= 0
      ? "Dry"
      : rateMMph < 2
        ? "Light"
        : rateMMph < 7
          ...
          : rateMMph < 20
            ? "Heavy"
            : "Very Heavy";
  const deltaPrefix = todayVsYesterday >= 0 ? "+" : "";
  ...
  return (
    <div className="wx-panel rainfall-panel">
      ...
      <div className="rainfall-highlights">
        <div className="rainfall-highlight-item">
          <span className="rainfall-highlight-label">Today vs Yesterday</span>
          <span className="rainfall-highlight-value">
            {deltaPrefix}{fmtRain(todayVsYesterday)} {unitLabel}
          </span>
        </div>
        <div className="rainfall-highlight-item">
          <span className="rainfall-highlight-label">Monthly Share of Year</span>
          <span className="rainfall-highlight-value">
            {yearlyProgress !== null ? `${yearlyProgress.toFixed(0)}%` : "--"}
          </span>
        </div>
      </div>
      ...
      <div className="rainfall-rate-footer">
        <span className="rainfall-rate-label">Current Rain Rate ({rainIntensity})</span>
